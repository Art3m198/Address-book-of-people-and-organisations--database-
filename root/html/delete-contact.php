		...
		if($contact->found) {
			...
			if(isset($_POST["submit"]) && $_POST["submit"] == "submit") {
				...
				if(isset($_POST["confirm_delete"])) {
					...
					if(empty($errors)) {
						$path = '../../uploads/persons/' . translit($contact->last_name . "_". $contact->first_name) . "_" . preg_replace($pattern, $replacement, $contact->added); // Директория для размещения файла
						rmRec($path);
						// Delete the contact
...
?>
			...
			<h3>Attention!</h3>
			<p><strong>This process is <u>IRREVERSIBLE</u>. After deleting a PERSON, the only way to restore it to the list is to manually add it again.</strong></p>
			<p>Please confirm <strong>the deletion action</strong> <?php echo $contact->full_name; ?> from the system.</p>

			<form class="form-horizontal" action="" method="post">

				<div class="checkbox">
					<label>
						<input type="checkbox" name="confirm_delete"> Yes, I'm sure I want to <strong>delete</strong> <?php echo $contact->full_name; ?>
					</label>
				</div>
				...
				<div >
					<button type="submit" name="submit" value="submit" class="btn btn-danger">Delete PERSON</button> <a href="index.php"<type="button" role="button" class="btn btn-info">Back</a>
				</div>
			</form>
			<!-- /CONTENT -->
