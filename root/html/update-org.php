<?php
	...
	if(isset($_GET['i'])) {
		...
		if($contact->found) {
			...
			$subpage_name = $contact->name . ' - ' . PAGENAME_ORGUPDATE;
			...
			if(!empty($_POST["name"]) && isset($_POST["name"])) 									{ $form_name = htmlentities($_POST["name"]); } 										elseif(!empty($contact->single["name"]) && isset($contact->single["name"]))											{ $form_name = htmlentities($contact->single["name"]); } 									else { $name = null; };
			if(!empty($_POST["head"]) && isset($_POST["head"])) 									{ $form_head = htmlentities($_POST["head"]); } 										elseif(!empty($contact->single["head"]) && isset($contact->single["head"]))											{ $form_head = htmlentities($contact->single["head"]); } 									else { $head = null; };
			...
			if(!empty($_POST["updated"]) && isset($_POST["updated"])) 									{ $form_updated = htmlentities($_POST["updated"]); } 									elseif(!empty($contact->single["updated"]) && isset($contact->single["updated"])) 										{ $form_updated = htmlentities($contact->single["updated"]); } 									else { $updated = null; };
		...
		$length_address_post_code = strlen($_POST["address_post_code"]);
				...
				if(empty($errors)) {
					...
					if(isset($_POST['first_name']) && !empty($_POST["first_name"])) 						{ $update_values['first_name'] = $_POST['first_name']; } else { $update_values['first_name'] = null; };
					...
					if(isset($_POST['name']) && !empty($_POST["name"])) 									{ $update_values['name'] = $_POST['name']; } else { $update_values['name'] = null; };
					if(isset($_POST['head']) && !empty($_POST["head"])) 									{ $update_values['head'] = $_POST['head']; } else { $update_values['head'] = null; };
					...
					if(isset($_POST['photo']) && !empty($_POST["photo"])) 									{ $update_values['photo'] = "../../uploads/orgs/" . translit($_POST['name']) . "_" . preg_replace($pattern, $replacement, $_POST['added']) . "/" . $_POST['photo']; } else { $update_values['photo'] = $form_photo; };
			...
			$subpage_name = 'Contact Not Found - ' . PAGENAME_ORGUPDATE;
		...
		$subpage_name = 'Invalid GET Value - ' . PAGENAME_ORGUPDATE;
		$session->message_alert($notification["contact"]["update"]["not_found"], "danger");
...
?>
			...
			<form class="form-horizontal" action="" enctype="multipart/form-data" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input type="text" readonly class="form-control" name="name" placeholder="Name" maxlength="1000" <?php if(!empty($form_name)) { echo "value=\"" . $form_name . "\""; }; ?>required>
					</div></div><hr>
					
					<div class="form-group">					
					<label class="col-sm-2 control-label">Head</label>
					<div class="col-sm-4">
						<input type="text" class="form-control" name="head" placeholder="Head" maxlength="1000" <?php if(!empty($form_head)) { echo "value=\"" . $form_head . "\""; }; ?> required>
					</div>
				</div>
								<hr>
									<div class="form-group">					
					...
					<div class="col-sm-4">
						<input type="text" class="form-control" name="city" placeholder="City" maxlength="200" <?php if(!empty($form_city)) { echo "value=\"" . $form_city . "\""; }; ?> required>
					</div>
				</div><hr>
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input type="text" class="form-control" name="web" placeholder="Social network" <?php if(!empty($form_web)) { echo "value=\"" . $form_web . "\""; }; ?>>
					</div></div><hr>
					...
					<div class="col-sm-4">
						<input type="text" class="form-control" name="added" placeholder="Date added" readonly <?php if(!empty($form_added)) { echo "value=\"" . $form_added . "\""; }; ?> required>
					</div>
					...
					<div class="col-sm-4">
						<input type="text" class="form-control" name="updated" placeholder="Update" readonly value="<?php echo date("d.m.Y H:i");?>" <?php if(!empty($form_updated)) { echo "value=\"" . $form_updated . "\""; }; ?> required>
					</div>
