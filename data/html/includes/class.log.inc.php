		...
		private function get_action($action = null, $additional_message = null) {
			...
			switch($action) {
				case 'view' : // For page views
					$action = 'Page view: (' . page_name() . ')'; // Use the page_name function to specify which page a user has visited
					break;
				...
				case 'not_authenticated' :
					$action = 'Unauthorized access attempt: (' . page_name() . ')';
					break;
				case 'user_add_failed' :
					$action = 'Error adding admin';
				...
				case 'user_add_success' :
					$action = 'Successful Admin addition';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'user_delete_failed' :
					$action = 'Admin deletion error';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'user_delete_success' :
					$action = 'Successful removal of Admin';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'user_update_failed' :
					$action = 'Admin update error';
				...
				case 'user_update_success' :
					$action = 'Successful admin update';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_add_failed' :
					$action = 'Error adding a Person';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_add_success' :
					$action = 'Persona added successfully';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_delete_failed' :
					$action = 'Error deleting a Person';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_delete_success' :
					$action = 'Successful deletion of a Person';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_update_failed' :
					$action = 'Person update error';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_update_success' :
					$action = 'Successful Persona update';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
