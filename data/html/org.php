...
?>
			<!-- CONTENT -->
			<p>To find an item in the database, start entering the name of the organization or city.</p><hr>
		...
		<div id="users">
			<div class="filter-group row">
				<div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-12">
					<input type="text" class="search form-control" placeholder="Start entering name or other information" />
				</div>
			 ...
			 foreach($contacts->all as $contact){
				...
				<div class="well well-sm col-lg-12 info-block block-info clearfix">
                    <div class="square-box pull-left">
                      <span> <a href="<?php echo PAGELINK_ORGVIEW; ?>?i=<?php echo urlencode($contact["contact_id"]); ?>"><img width="160" height="160" class="img-responsive" src='<?php echo $contact["photo"] ?>'></a>   </span>
                    </div>
					<p class="name"><b><a href="<?php echo PAGELINK_ORGVIEW; ?>?i=<?php echo urlencode($contact["contact_id"]); ?>"><?php echo htmlentities($contact["name"]); ?></a></b></p4>
					<h5 class="city">City: <?php echo htmlentities($contact["city"]); ?></h5>
					<h5 class="head">Head: <?php echo htmlentities($contact["head"]); ?></h5>
					</div>
				</li>
						<?php $counter++;
				// Closing the foreach loop once final item in $contacts has been displayed
					...
					?>
						...
						<div class="col-xs-12 col-sm-6 col-md-6 col-lg-12 pagination"></div>
		</div>
	</div>
</div> 	
