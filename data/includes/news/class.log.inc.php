		...
		private function get_action($action = null, $additional_message = null) {
			...
			switch($action) {
				case 'view' : // For page views
					$action = 'Просмотр страницы: (' . page_name() . ')'; // Use the page_name function to specify which page a user has visited
					break;
				case 'not_found' : // For accessing pages which couldn't be found, such as invalid $_GET values or values which couldn't be found in the database
					$action = "Результат не найден: (" . page_name() . ')';
					break;
				case 'login_failed' :
					$action = 'Ошибка входа';
				...
				case 'not_authenticated' :
					$action = 'Неавторизованная попытка доступа: (' . page_name() . ')';
					break;
				case 'user_add_failed' :
					$action = 'Ошибка добавления админа';
				...
				case 'user_add_success' :
					$action = 'Успешное добавление Админа';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'user_delete_failed' :
					$action = 'Ошибка удаления Админа';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'user_delete_success' :
					$action = 'Успешное удаление Админа';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'user_update_failed' :
					$action = 'Ошибка обновления админа';
				...
				case 'user_update_success' :
					$action = 'Успешное обновление админа';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_add_failed' :
					$action = 'Ошибка добавления Персоны';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_add_success' :
					$action = 'Персона успешно добавлена';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_delete_failed' :
					$action = 'Ошибка удаления Персоны';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_delete_success' :
					$action = 'Успешное удаление Персоны';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
				case 'contact_update_failed' :
					$action = 'Ошибка обновления Персоны';
					if($additional_message == 'database') {
						$action .= ': There was an error making changes to the database.';
					} elseif($additional_message) {
						$action .= ': ' . $additional_message;
				...
				case 'contact_update_success' :
					$action = 'Успешнон обновление Персоны';
					if($additional_message) {
						$action .= ': ' . $additional_message;
					};
					break;
